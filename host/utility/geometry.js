export function calculatePolygonHeight(sides, radius) {
...
export function distanceBetweenVertices({ sides, radius, distance, }) {
    if (sides < 3) {
...
export function calculateShortestWidthOfPolygon(sides, radius) {
    if (sides < 3) {
        throw new Error('A polygon must have at least 3 sides.');
    }
    if (sides % 2 === 0) {
...
export function calculateIncircleRadius(sides, outerRadius) {
    if (sides < 3) {
        throw new Error('A polygon must have at least 3 sides.');
    }
...
export function calculatePolygonSideLength(sides, radius) {
    if (sides < 3) {
        throw new Error('A polygon must have at least 3 sides.');
    }
...
export function calculateCircumradiusFromIncircleRadius(sides, radius) {
    if (sides < 3) {
        throw new Error('A polygon must have at least 3 sides.');
    }
    return radius / Math.cos(Math.PI / sides);
}
export function calculatePolygonVertices(sides, radius) {
    const vertices = [];
...
export function calculateSideLength(radius, sides) {
...
export function normalVector(p1, p2) {
...
export function calculateTrianglePositions(vertices, sideIndex, triangles, height) {
...
export function calculatePolygonDotPosition({ polygonRadius, polygonSideCount, polygonEdgeNumber, polygonEdgePositionRatio, // from 0 to 1
gap = 0, dotRadius, rotation = 0, offset = 0, flatSide, }) {
...
export function getPolygonVertex(i, n, R) {
...
export const POLYGON_SIDE = ['left', 'right', 'top', 'bottom'];
export function computePolygonPoints({ width, height, sides, strokeWidth, rotation = 0, flatSide, }) {
