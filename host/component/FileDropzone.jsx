...
import { INPUT_COLOR, INPUT_WAITING, getRoundedClass, } from './Input';
import { useText } from './Text';
import { useAppSettings } from '../hook/useAppSettings';
// https://react-dropzone.js.org/
export default function FileDropzone({ color, children, className, accept, multiple, onDrop, topped, bottomed, borderClassName, font = 'Noto Sans Mono', }) {
    ...
    return (<div className={cx(className, roundedClassName, backgroundColorClass, textColorClass, 'p-16 transition-all duration-200 relative cursor-pointer hover:opacity-70')} {...getRootProps()}>
      <input {...getInputProps()}/>
      <div className={cx('transition-all duration-200', 'absolute top-0 bottom-0 right-0 left-0 border-4', isDragActive
            ? cx(borderClassName, roundedClassName, `border-dashed`)
            : undefined)}/>
      {children}
    </div>);
}
//# sourceMappingURL=FileDropzone.jsx.map