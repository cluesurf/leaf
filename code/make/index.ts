...
export class Make {
  ...
  async save(): Promise<MakeBack & { code: string }> {
    ...
    // Build the bundled `Code` type. Includes the inline
    // `CodeLink` integer-id table at the bottom of the same
    // file.
...
function makeCode(books: Book[]): string {
  ...
  const colonKeys: string[] = []   // captured for the CodeLink table below
  ...
  // Emit the integer-id table (`CodeLink`) inline. Sorted
  // lexicographically + 1-based for deterministic ids across
  // re-runs. Compile target for `base.call(...)` rewrites.
  const sortedKeys = [...new Set(colonKeys)].sort()
  lines.push('export const CodeLink = {')
  for (let i = 0; i < sortedKeys.length; i++) {
    lines.push(`  '${sortedKeys[i]}': ${i + 1},`)
  }
  lines.push('} as const')
  lines.push('')
  lines.push('export type CodeLink = (typeof CodeLink)[keyof typeof CodeLink]')
  lines.push('')
